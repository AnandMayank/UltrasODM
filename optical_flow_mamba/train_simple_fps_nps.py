"""
Simplified FPS/NPS Training Script

Based on TUS-REC baseline approach with FPS/NPS enhancements.
Focus: Simple, effective training with PointDistance loss only.
"""
...
def setup_data(config, device):
    ...
    # Split dataset (following TUS-REC baseline approach)
    ...
    # Setup transformations (borrowed from main utils)
...
def train_epoch(model, criterion, train_loader, optimizer, device,
    ...
    for step, (frames, tforms, _, _) in enumerate(train_loader):
        ...
        # Debug: Print output range for first few steps
        if step < 3:
            print(f"Step {step} - Outputs range: [{outputs.min().item():.4f}, {outputs.max().item():.4f}]")
            print(f"Rotation angles (first 3): {outputs[0, :3].detach().cpu().numpy()}")

        # Clamp outputs to reasonable range for Euler angles (more conservative)
        outputs = torch.clamp(outputs, -3.14, 3.14)  # Within [-π, π] for angles
...
def validate_epoch(model, criterion, val_loader, device,
    ...
    with torch.no_grad():
        for frames, tforms, _, _ in val_loader:
            ...
            # Debug: Check for invalid outputs
            if torch.isnan(outputs).any() or torch.isinf(outputs).any():
                print(f"Warning: Invalid validation outputs detected")
                continue
...
def main():
    ...
    print("[START] Starting Simplified FPS/NPS Training")
    print("=" * 60)
    ...
    num_pairs = data_pairs.shape[0]  # Calculate number of pairs
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        if (epoch + 1) % config.VAL_FREQUENCY == 0:
            ...
            if val_distance < best_val_distance:
                ...
                print(f"[OK] New best model saved! Distance: {val_distance:.4f}mm")
