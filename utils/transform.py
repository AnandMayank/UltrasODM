...
class PredictionTransform():
    ...
    def __call__(self, outputs):
        # Handle both (B, 6) and (B, T, 6) outputs
        if outputs.dim() == 2:
            # Standard case: (B, 6) -> (B, num_pairs, 6)
            preds = outputs.reshape((outputs.shape[0], self.num_pairs, -1))
        elif outputs.dim() == 3:
            # Sequential case: (B, T, 6) -> use as is, treating T as sequence dimension
            preds = outputs
        else:
    ...
    def param_to_transform(self,params):
        # params: (batch,ch,6), "ch": num_pairs or num_frames, "6":rx,ry,rz,tx,ty,tz
        matrix = pytorch3d.transforms.euler_angles_to_matrix(params[...,0:3], 'ZYX')

        # Fix tensor shape mismatch: ensure translation has correct shape
        translation = params[...,3:6]  # Explicitly take only 3 translation components
        if translation.shape[-1] != 3:
            raise ValueError(f"Expected 3 translation components, got {translation.shape[-1]}")

        # Add dimension for proper concatenation: (B, seq_len, 3) -> (B, seq_len, 3, 1)
        translation_expanded = translation.unsqueeze(-1)

        transform = torch.cat((matrix, translation_expanded), axis=3)

        # Handle both (B, num_pairs, 6) and (B, num_frames, 6) cases
        batch_size, seq_len = matrix.shape[0], matrix.shape[1]

        # Create last rows with proper shape
        zeros_shape = (batch_size, seq_len, 1, 3)
        ones_shape = (batch_size, seq_len, 1, 1)

        zeros = torch.zeros(zeros_shape, device=matrix.device, dtype=matrix.dtype)
        ones = torch.ones(ones_shape, device=matrix.device, dtype=matrix.dtype)

        last_rows = torch.cat((zeros, ones), axis=3)  # (B, seq_len, 1, 4)
