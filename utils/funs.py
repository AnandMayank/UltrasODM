...
def add_scalars(writer,epoch, loss_dists):
    ...
    epoch_dist_val = loss_dists['val_epoch_dist']
...
def load_config(file_path):
    """Load the config file with section support
    
    Args:
        file_path: Path to the config file
        
    Returns:
        tuple: (dict of args, argparse.Namespace of args)
    """
    args = {}
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if not line or line.startswith('===') or line.startswith('[') or \
               line.startswith('Created:') or line.startswith('//'):
                continue
            ...
            try:
                # Split on first colon and handle possible spaces
    ...
    defaults = {
        'FREQ_INFO': 100,
        'FREQ_SAVE': 100,
        'NUM_EPOCHS': int(1e8),  # From your config
        'MINIBATCH_SIZE': 10,
        'LEARNING_RATE': 0.0001,
        'NUM_PRED': 1,
        'NUM_SAMPLES': 8,
        'SAMPLE_RANGE': 20,
        # UltrasOM specific parameters
        'EMBED_DIM': 256,
        'NUM_LAYERS': 4,
        'PRED_DIM': 6,
        'BATCH_SIZE': 8,
    ...
    # Copy missing defaults and convert types
    for key, default in defaults.items():
        try:
            if key in args:
                # Special handling for SAMPLE_RANGE which might be a list
                if key == 'SAMPLE_RANGE':
                    value = args[key]
                    if value.startswith('[') and value.endswith(']'):
                        # Parse list format like "[0, -1]"
                        value = value.strip('[]').split(',')
                        args[key] = [int(v.strip()) for v in value if v.strip()]
                    else:
                        args[key] = int(value)
                else:
                    # Convert existing value to proper type
                    args[key] = type(default)(args[key])
            else:
                # Use default if key is missing
                args[key] = default
        except ValueError as e:
            print(f"Warning: Could not convert {key}, using default {default}")
            args[key] = default
